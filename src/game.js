...
export function newGame(names,rounds=6,difficulty='classic'){
 ...
 return {names:names.map((n,i)=>n.trim()||`Explorer ${i+1}`),rounds,difficulty,deck:shuffle(countries).slice(0,rounds*2).map(c=>({country:c,options:shuffle([c,...shuffle(countries.filter(x=>x.id!==c.id&&x.capital!==c.capital)).slice(0,count-1)])})),turn:0,scores:[0,0],stage:'handoff',pot:0,feedback:null};
}
...
export function transition(s,a){
 ...
 if(a.type==='finish'&&s.stage==='recap'){
  const scores=[...s.scores];scores[s.turn%2]+=s.pot;const turn=s.turn+1;
